...
pub struct Sp1MemoryBusInteraction<V> {
    addr: MemoryAddress<[GroupedExpression<KoalaBearField, V>; 3]>,
...
#[derive(
    Clone, Hash, Eq, PartialEq, Serialize, Deserialize, Debug, Copy, deepsize2::DeepSizeOf,
)]
...
pub struct MemoryAddress<E> {
    pub address_space: ArtificialAddressSpace,
    /// The memory address, represented as 3 16-Bit limbs in little-endian order.
    pub addr: E,
}

impl<V: Ord + Clone + Eq + Display + Hash>
    MemoryAddress<[GroupedExpression<KoalaBearField, V>; 3]>
{
...
impl<V: Ord + Clone + Eq + Display + Hash> IntoIterator
    for MemoryAddress<[GroupedExpression<KoalaBearField, V>; 3]>
{
...
{
    type Address = MemoryAddress<[GroupedExpression<KoalaBearField, V>; 3]>;
