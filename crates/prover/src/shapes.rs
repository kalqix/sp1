...
/// (including any APCs). Use this when the baked-in compress_shape.json may not be sufficient.
pub async fn compute_compress_shape(
    machine: Machine<SP1Field, RiscvAir<SP1Field>>,
    elf: &[u8],
) -> SP1RecursionProofShape {
    ...
    for cluster in chip_clusters {
        ...
        let program =
            normalize_program_from_input(&recursive_core_verifier, &shape.dummy_input(vk.clone()));
...
mod tests {
    ...
    async fn test_find_recursion_shape() {
        setup_logger();
        let machine = RiscvAir::machine();
        let shape = compute_compress_shape(machine, &test_artifacts::FIBONACCI_ELF).await;
